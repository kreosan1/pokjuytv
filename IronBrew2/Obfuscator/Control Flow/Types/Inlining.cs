			...
			foreach (Chunk sub in chunk.Functions.ToList())
			{
				...
				if (ShouldInline(chunk, sub, out var locations, out var closures, out bool inlineAll))
				{
					...
					foreach (var loc in locations)
					{
						...
						Instruction next = chunk.Instructions.Count > idx ? chunk.Instructions[idx] : null;
								...
								case Opcode.Return:
									{
										...
										if (callLimit <= -1)
											callLimit = Math.Max(0, sub.StackSize - instr.A);
								...
								case Opcode.TailCall:
									{
										...
										if (callLimit == -1)
											callLimit = Math.Max(0, chunk.StackSize - loc.A + 1);
					...
					{
						if (clos.Chunk != chunk || !chunk.InstructionMap.ContainsKey(clos) || clos.RefOperands[0] is not Chunk cChunk)
							continue;

						int closIdx = chunk.InstructionMap[clos];
						int removeLength = cChunk.UpvalueCount + 1;
						if (closIdx + removeLength <= chunk.Instructions.Count)
							chunk.Instructions.RemoveRange(closIdx, removeLength);
