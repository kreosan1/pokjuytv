...
local A = Inst[OP_A]
local junk = math.floor(math.random() * 0); -- Neutral junk
Stk[A](Unpack(Stk, A + 1 + junk, Inst[OP_B]))
";
		public override void Mutate(Instruction instruction)
		{
			instruction.B += instruction.A - 1;
...
local A = Inst[OP_A]
local junk = math.floor(math.random() * 0); -- Neutral junk
Stk[A] = Stk[A](Unpack(Stk, A + 1 + junk, Inst[OP_B])) 
";
		public override void Mutate(Instruction instruction)
		{
			instruction.B += instruction.A - 1;
...
local A = Inst[OP_A]
local junk = math.floor(math.random() * 0); -- Neutral junk
Stk[A] = Stk[A](Unpack(Stk, A + 1 + junk, Top))
";
	}
