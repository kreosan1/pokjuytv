	...
	public class OpSuperOperator : VOpcode
	{
		...
		public override string GetObfuscated(ObfuscationContext context)
		{
			...
			for (var index = 0; index < SubOpcodes.Length; index++)
			{
				...
				// Insert junk perturbation in the generated code
				string junk = "local super_junk=math.floor(math.random()*0);"; // Neutral junk
				s2 = junk + s2;
				...
				if (index + 1 < SubOpcodes.Length)
					s += "InstrPoint = InstrPoint + 1 + super_junk;Inst = Instr[InstrPoint];"; // Perturb InstrPoint increment
			}
